...
class ProblemRepository
{
    ...
    {
        $stmt = $this->db->query('SELECT id, title FROM problems ORDER BY id');
    ...
    public function findById(int $id): ?array
    {
        $stmt = $this->db->prepare('
            SELECT id, title, statement_md, template_code, holes_json, tests_json, time_limit_ms, memory_limit_kb
            FROM problems
            WHERE id = ?
        ');
        $stmt->execute([$id]);
